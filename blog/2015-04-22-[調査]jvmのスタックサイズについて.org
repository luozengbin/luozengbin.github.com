...
- JVMランチャーやJavaAPIから起動されたJavaスレッドのスタックサイズは  =-Xss= もしくは
...
JVMスタックに関して、公式のJVMスペックドキュメント ([[https://docs.oracle.com/javase/specs/jvms/se7/html/][Java SE 7 Virtual Machine
Specification]]) は次のように記載されています。

- [[http://docs.oracle.com/javase/specs/jvms/se7/html/jvms-2.html#jvms-2.5.2][2.5.2. Java Virtual Machine Stacks]]
  #+begin_example
...
- [[http://docs.oracle.com/javase/specs/jvms/se7/html/jvms-2.html#jvms-2.5.6][2.5.6. Native Method Stacks]]
  #+begin_example
...
理論上は =Java Stack= と =Native Stack= がスレッド毎に領域が確保されいます。ただし、
実際のメモリページ構成はJDKの実装に依存するものです。

次の情報によると、HotSpotの実装は =Java Stack= と =Native Stack= が同じメモリ領域を共
有してる。

- [[http://docs.oracle.com/javase/7/docs/webnotes/tsg/TSG-VM/html/toc.html][Troubleshooting Guide for HotSpot VM]] の [[https://docs.oracle.com/javase/7/docs/webnotes/tsg/TSG-VM/html/crashes.html#gbyzz][4.1.3 Crash due to Stack Overflow]]
  #+begin_example
...
- OpenJDKの開発メーリングリストから拾った内容
  ...
  #+end_example
  情報源： [[http://mail.openjdk.java.net/pipermail/hotspot-dev/2011-June/004272.html][What the difference between -Xss and -XX:ThreadStackSize is?]]
...
** glibc guard page
=glibc guard page= はスタックポインタのオーバーフローを防ぐための－ガードページ。Java
スレッドには =HotSpot Guard Pages= が別途用意されているため、この領域のサイズが0であ
る。非Javaスレッドはスタック頂上位置に1ページ分のガードページが割り当てられる。以下は
その実装内容です。

- スレッド作成時にglibcの [[http://linuxjm.sourceforge.jp/html/LDP_man-pages/man3/pthread_attr_setguardsize.3.html][pthread_attr_setguardsize]] 関数にてガードページを作成してい
  る

  [[http://hg.openjdk.java.net/jdk7/jdk7/hotspot/file/9b0ca45cd756/src/os/linux/vm/os_linux.cpp#l923][jdk7/hotspot/src/os/linux/vm/os_linux.cpp#l923]]
  #+begin_src c++
...
- スレッド種別によってガードページのサイズを決める
  ...
  [[http://hg.openjdk.java.net/jdk7/jdk7/hotspot/file/9b0ca45cd756/src/os_cpu/linux_x86/vm/os_linux_x86.cpp#l662][jdk7/hotspot/src/os_cpu/linux_x86/vm/os_linux_x86.cpp#l662]]
  #+begin_src c++
...
- [[http://docs.oracle.com/cd/E19455-01/806-2732/attrib-22/index.html][スタックガードの大きさの設定]]
...
Pages= 領域がスタック領域のトップ位置に設けている。また =HotSpot Guard Pages= は
=StackYellowPages= と =StackRedPages= から構成されている。

=StackYellowPages= はスタックオーバーフローの緩衝域として、余分のメモリーを割り当てま
す。 スタックポインターが =StackRedPages= まで行くとStackOverflowErrorが起きる。
...
それぞれの値は =-XX:StackYellowPages= と =-XX:StackRedPages= パラメータにて変更するこ
とが可能です。
...
以下は =JBoss AS7= アプリケーションサーバ実行時にワーカスレッドのスタック仮想メモリ割
当状況です。
