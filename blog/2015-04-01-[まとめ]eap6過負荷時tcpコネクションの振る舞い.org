...
以下の状況に置いて、クライアントからのリクエスデータ[1]がサーバ側のTCPバッファーに詰ま
る。TCPバッファー[2]が一杯になると、パケットがdropされるのでクライアントからのデータ再
送が発生する。
...
* 参考
[1] HTTPリクエスト電文のヘッダ部やボディ部の内容
[2] TCP接続単位にTCPバッファーが持っている、デフォルト値は
    net.ipv4.tcp_rmemのdefaultパラメータ値の1/2になります、約42KB。
    https://github.com/torvalds/linux/blob/master/net/ipv4/tcp.c#L3072
